...
class DBC implements IteratorAggregate {
    ...
    /**
     * Fetches a field from the DBC, given the record's ID (first field), and ensures the index is generated
     */
    public function getFieldFromRecord($recordId, $fieldName, $fieldType = DBC::UINT) {
        if($this->_index === null)
            $this->index();
        if(isset($this->_index[$recordId])) {
            $record = new DBCRecord($this, $this->_index[$recordId]);
            $ret = $record->get($fieldName, $fieldType);
            unset($record);
            return $ret;
        }
        return null;
